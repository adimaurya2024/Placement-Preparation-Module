...
public:
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        deque<int> d1;
        vector<int> ans;
        // process the k elements in the decreasing order in the dequeue 
        for ( int i = 0 ; i < k ; i++)
        {
            int element = nums[i];
            ...
            // finally push the current element into the dequeue 
            d1.push_back(i);
        }
        for( int i = k ; i < nums.size(); i++)
        {
            ...
            {
                d1.pop_front();
                // popping in order to make decreasing dequeue
            }
